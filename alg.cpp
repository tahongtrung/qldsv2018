			...
			case 13:{
				if(tt==n || strlen(stemp[k+1]) > 0){//enter len xg d
					if(checkFull(stemp,n)){
						...
						if(ck==4){
							if(checkMaMH(root_mh,stemp[0])==0){
							...
							strcpy(mh.MAMH,stemp[0]);
							strcpy(mh.TENMH,stemp[1]);
							mh.STCLT = atoi(stemp[2]);
							mh.STCTH = atoi(stemp[3]);
							InsertNodeMH(root_mh,mh);
							//exit(mh.STCTH);
							//dsmh.n++;
						}
...
int checkMaMH(NODEMH root,char token[]){
	...
	if (strcmp(root->mh.MAMH,token) == 0)
	{
		return 0;
	}
	if (strcmp(root->mh.MAMH,token)<0)
	{
		checkMaMH(root->left, token);
	}
	else
	{
		checkMaMH(root->right, token);
	}
...
void InDSMH(NODEMH root,int sp,int i){
	if (root != NULL)
	{
		...
		GotoXY(sp,i+6); printf("%s",root->mh.MAMH);
		GotoXY(sp+sp,i+6); printf("%s",root->mh.TENMH);
		GotoXY(sp+sp+sp,i+6); printf("%d",root->mh.STCLT);
...
void InsertNodeMH(NODEMH &p, _monhoc mh){
	if(p == NULL){
		p = new nodemh;
		p->mh=mh;
//		strcpy(p->mh.MAMH,mh.MAMH);
//		strcpy(p->mh.TENMH,mh.TENMH);
//		
//		p->mh.STCLT = mh.STCLT;
//		p->mh.STCTH = mh.STCTH;
		...
		if(strcmp(strupr(mh.MAMH),strupr(p->mh.MAMH))<0)
			InsertNodeMH(p->left,mh);
		else
			InsertNodeMH(p->right,mh);
	}
}
...
void readFileMonHoc(NODEMH dsmh){
	...
	if(fp == NULL){
		...
		while(!feof(fp)){
			...
			fgets(mh.MAMH,255,fp);
			strtok(mh.MAMH,"\n");
			
			fgets(mh.TENMH,255,fp);
			strtok(mh.TENMH,"\n");
...
//void writeFileMonHoc(FILE *fp,NODEPTR p,char malop[]){
//	char path[]="bin\\subject.ini";
//	FILE *fp = fopen(path,"w+");
//	if(fp == NULL) printf("Error write file!");
//	else{
...
//	fclose(fp);
//}
void writeFileMH(FILE *fp,NODEMH p){
	if(p!=NULL){	
		fprintf(fp,"%s\n",strupr(p->mh.MAMH));
		fprintf(fp,"%s\n",strupr(p->mh.TENMH));
		fprintf(fp,"%d\n",(p->mh.STCLT));
		fprintf(fp,"%d\n",(p->mh.STCTH));	
		writeFileMH(fp,p->left);
		writeFileMH(fp,p->right);
	}
}
void writeFileMonHoc(NODEMH dsmh){
	char path[]="bin\\subject.ini";
	FILE *fp = fopen(path,"w+");
	if(fp == NULL) printf("Error write file!");
	else{
...
void ReadFile(){
	readFileMonHoc(root_mh);
	readFileLop(dsl);
	readFileSinhVien(dsl);
	readFileDiem(dsl);
}
void WriteFile(){
	writeFileMonHoc(root_mh);
	writeFileLop(dsl);
